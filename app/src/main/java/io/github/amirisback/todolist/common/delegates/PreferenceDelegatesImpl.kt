...
import io.github.amirisback.todolist.common.ext.executePreference
import io.github.amirisback.todolist.common.ext.fetchPreference
import io.github.amirisback.todolist.common.ext.singleGetSharedPreferences
...
class PreferenceDelegatesImpl(context: Context, prefName: String) : PreferenceDelegates {
    ...
    override fun loadPrefString(key: String, callback: DataResponseCallback<String>) {
        Observable.just(loadPrefString(key)).fetchPreference(callback)
    }

    override fun loadPrefLong(key: String, callback: DataResponseCallback<Long>) {
        Observable.just(loadPrefLong(key)).fetchPreference(callback)
    }

    override fun loadPrefFloat(key: String, callback: DataResponseCallback<Float>) {
        Observable.just(loadPrefFloat(key)).fetchPreference(callback)
    }

    override fun loadPrefInt(key: String, callback: DataResponseCallback<Int>) {
        Observable.just(loadPrefInt(key)).fetchPreference(callback)
    }

    override fun loadPrefBoolean(key: String, callback: DataResponseCallback<Boolean>) {
        Observable.just(loadPrefBoolean(key)).fetchPreference(callback)
    }

    override fun savePrefString(key: String, value: String, callback: StateResponseCallback) {
        Completable
            .fromAction { savePrefString(key, value) }
            .executePreference(callback)
    }

    override fun savePrefLong(key: String, value: Long, callback: StateResponseCallback) {
        Completable
            .fromAction { savePrefLong(key, value) }
            .executePreference(callback)
    }

    override fun savePrefFloat(key: String, value: Float, callback: StateResponseCallback) {
        Completable
            .fromAction { savePrefFloat(key, value) }
            .executePreference(callback)
    }

    override fun savePrefInt(key: String, value: Int, callback: StateResponseCallback) {
        Completable
            .fromAction { savePrefInt(key, value) }
            .executePreference(callback)
    }

    override fun savePrefBoolean(key: String, value: Boolean, callback: StateResponseCallback) {
        Completable
